...
!!TODO: Check valid setup
        ...
        CASE(EQUATIONS_SET_SETUP_EQUATIONS_TYPE)
          ...
          CASE DEFAULT
            LOCAL_ERROR="The equation set subtype of "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%SPECIFICATION(3),"*",ERR,ERROR))// &
              & " for a setup of "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET_SETUP%SETUP_TYPE,"*",ERR,ERROR))// &
              & " is invalid for a Darcy equation."
            CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
          ...
          IF(DEPENDENT_FIELD%SCALINGS%SCALING_TYPE/=FIELD_NO_SCALING) THEN
            ...
            DO mh=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
              ...
              DO ms=1,DEPENDENT_BASIS_1%NUMBER_OF_ELEMENT_PARAMETERS
                mhs=mhs+1
                nhs=0
                IF(ASSOCIATED(STIFFNESS_MATRIX).AND.ASSOCIATED(DAMPING_MATRIX)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_DARCY)) THEN
                    ...
                    IF(ASSOCIATED(EQUATIONS_SET_DARCY)) THEN
                      ...
                      SELECT CASE(EQUATIONS_SET_DARCY%SPECIFICATION(3))
                        ...
                        CASE DEFAULT
                          LOCAL_ERROR="Equations set subtype " &
                            & //TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET_DARCY%SPECIFICATION(3),"*",ERR,ERROR))// &
                            & " is not valid for a Darcy equation fluid type of a fluid mechanics problem class."
                          CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
                      END SELECT
                    ELSE
...
999 ERRORSEXITS("DARCY_EQUATION_POST_SOLVE_SET_MASS_INCREASE",ERR,ERROR)
  ...
  SUBROUTINE DARCY_EQUATION_IMPERMEABLE_BC_VIA_PENALTY(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    EQUATIONS=>EQUATIONS_SET%EQUATIONS
    DYNAMIC_MATRICES=>EQUATIONS%equationsMatrices%DYNAMIC_MATRICES
    STIFFNESS_MATRIX=>DYNAMIC_MATRICES%MATRICES(1)%PTR
    DEPENDENT_FIELD=>EQUATIONS%INTERPOLATION%DEPENDENT_FIELD
    DECOMPOSITION  =>DEPENDENT_FIELD%DECOMPOSITION
...
!     MESH_COMPONENT_NUMBER=DECOMPOSITION%MESH_COMPONENT_NUMBER
    MESH_COMPONENT_NUMBER = EQUATIONS%equationsMapping%DYNAMIC_MAPPING%EQUATIONS_MATRIX_TO_VAR_MAPS(1)% &
