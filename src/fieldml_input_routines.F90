...
999 ERRORSEXITS( "FIELDML_INPUT_CHECK_FIELD_COMPATIBLE", ERR, ERROR )
  ...
  SUBROUTINE FieldmlInput_CoordinateSystemCreateStart( FIELDML_INFO, EVALUATOR_NAME, coordinateSystems, COORDINATE_SYSTEM, & 
    ...
    CALL CoordinateSystem_CreateStart( USER_NUMBER, coordinateSystems, COORDINATE_SYSTEM, ERR, ERROR, *999 )
    !Set the coordinate system dimension and type
    CALL CoordinateSystem_DimensionSet( COORDINATE_SYSTEM, COORDINATE_COUNT, ERR, ERROR, *999 )
    CALL CoordinateSystem_TypeSet( COORDINATE_SYSTEM, COORDINATE_TYPE, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_INPUT_NODES_CREATE_START", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_INPUT_MESH_CREATE_START( FIELDML_INFO, MESH_ARGUMENT_NAME, MESH, MESH_NUMBER, REGION, ERR, ERROR, * )
    ...
    NULLIFY( MESH )
    CALL Mesh_CreateStart(MESH_NUMBER, REGION, XI_DIMENSIONS, MESH, ERR, ERROR, *999 )
    CALL Mesh_NumberOfElementsSet( MESH, ELEMENT_COUNT, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_INPUT_MESH_CREATE_START", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_INPUT_BASIS_CREATE_START( FIELDML_INFO, EVALUATOR_NAME, USER_NUMBER, basisFunctions, BASIS, ERR, ERROR, * )
    ...
    NULLIFY(BASIS)
    CALL Basis_CreateStart(USER_NUMBER, basisFunctions, BASIS, ERR, ERROR, *999 )
    CALL Basis_TypeSet( BASIS, FIELDBASISTYPE, ERR, ERROR, *999 )
    CALL Basis_NumberOfXiSet( BASIS, size( BASIS_INTERPOLATIONS ), ERR, ERROR, *999 )
    CALL Basis_InterpolationXiSet( BASIS, BASIS_INTERPOLATIONS, ERR, ERROR, *999 )
    ...
    IF( size( BASIS_INTERPOLATIONS ) > 1 .AND. ALLOCATED(COLLAPSE)) THEN
      CALL Basis_CollapsedXiSet( BASIS, COLLAPSE, ERR, ERROR, *999 )
    ENDIF
...
999 ERRORSEXITS( "FIELDML_INPUT_REORDER", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_INPUT_CREATE_MESH_COMPONENT( FIELDML_INFO, MESH, COMPONENT_NUMBER, EVALUATOR_NAME, ERR, ERROR, * )
    ...
    TYPE(IntegerCIntAllocType), ALLOCATABLE :: CONNECTIVITY_ORDERS(:)
    ...
    DO ELEMENT_NUMBER = 1, ELEMENT_COUNT
      ...
      IF( BASIS_REFERENCE_HANDLE /= LAST_BASIS_HANDLE ) THEN
        ...
        CALL Basis_UserNumberFind(basisFunctions, BASIS_NUMBER, BASIS, ERR, ERROR, *999 )
      ...
      IF( ELEMENT_NUMBER == 1 ) THEN
        CALL MeshElements_CreateStart( MESH, COMPONENT_NUMBER, BASIS, MESH_ELEMENTS, ERR, ERROR, *999 )
      ENDIF
      
      CALL MeshElements_ElementBasisSet( MESH_ELEMENTS, ELEMENT_NUMBER, BASIS, ERR, ERROR, *999 )
      ...
      DO KNOWN_BASIS_NUMBER = 1, KNOWN_BASIS_COUNT
        ...
        IF( TEMP_BASIS_HANDLE == BASIS_REFERENCE_HANDLE ) THEN
          ...
          CALL MeshElements_ElementNodesSet( MESH_ELEMENTS, ELEMENT_NUMBER, NODES_BUFFER(1:BASIS_NODES_COUNT), &
            & ERR, ERROR, *999 )
        ENDIF
      ENDDO
...
999 ERRORSEXITS( "FIELDML_INPUT_CREATE_MESH_COMPONENT", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_INPUT_FIELD_CREATE_START( FIELDML_INFO, REGION, DECOMPOSITION, FIELD_NUMBER, FIELD, VARIABLE_TYPE, &
    ...
    NULLIFY( FIELD )
    CALL Field_CreateStart( FIELD_NUMBER, REGION, FIELD, ERR, ERROR, *999 )
    CALL Field_TypeSet( FIELD, FIELD_GEOMETRIC_TYPE, ERR, ERROR, *999 )
    CALL Field_DecompositionSet( FIELD, DECOMPOSITION, ERR, ERROR, *999 )
    CALL Field_ScalingTypeSet( FIELD, FIELD_NO_SCALING, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_INPUT_FIELD_CREATE_START", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_INPUT_FIELD_PARAMETERS_UPDATE(FIELDML_INFO, EVALUATOR_NAME, FIELD, VARIABLE_TYPE, SET_TYPE, &
    ...
    IF(ASSOCIATED(FIELD)) THEN
      CALL Field_NumberOfComponentsGet(FIELD,VARIABLE_TYPE,NUMBER_OF_COMPONENTS,ERR,ERROR,*999)
      IF(NUMBER_OF_COMPONENTS>0) THEN
        CALL Field_ComponentInterpolationGet(FIELD,VARIABLE_TYPE,1,INTERPOLATION_TYPE,ERR,ERROR,*999)
        CALL Field_ComponentMeshComponentGet(FIELD,VARIABLE_TYPE,1,MESH_COMPONENT1,ERR,ERROR,*999)
        ...
        DO component_idx=2,NUMBER_OF_COMPONENTS
          CALL Field_ComponentInterpolationGet(FIELD,VARIABLE_TYPE,component_idx,INTERPOLATION_TYPE,ERR,ERROR,*999)
          CALL Field_ComponentMeshComponentGet(FIELD,VARIABLE_TYPE,component_idx,MESH_COMPONENT2,ERR,ERROR,*999)
...
999 ERRORSEXITS("FIELDML_INPUT_FIELD_PARAMETERS_UPDATE",ERR,ERROR)
  ...
  SUBROUTINE FieldmlInput_FieldNodalParametersUpdate( FIELDML_INFO, EVALUATOR_NAME, FIELD, VARIABLE_TYPE, SET_TYPE, &
    ...
    CALL Field_NumberOfComponentsGet( FIELD, VARIABLE_TYPE, FIELD_DIMENSIONS, ERR, ERROR, *999 )
    ...
    DO NODE_NUMBER = 1, MESH_NODE_COUNT
      ...
      DO COMPONENT_NUMBER = 1, FIELD_DIMENSIONS
        ...
        CALL Decomposition_NodeDomainGet(FIELD%DECOMPOSITION,NODE_NUMBER,meshComponentNumber,nodeDomain,err,error,*999)
        IF(nodeDomain==myWorldComputationNodeNumber) THEN
          CALL Field_ParameterSetUpdateNode( FIELD, VARIABLE_TYPE, SET_TYPE, VERSION_NUMBER, &
