...
CONTAINS
  ...
  SUBROUTINE Poisson_BoundaryConditionsAnalyticCalculate(EQUATIONS_SET,BOUNDARY_CONDITIONS,err,error,*)
    ...
    !Local Variables
    INTEGER(INTG) :: component_idx,deriv_idx,dim_idx,local_ny,node_idx,numberOfDimensions,variable_idx,variable_type,global_ny
    REAL(DP) :: VALUE,X(3)
        ...
        IF(ASSOCIATED(dependentField)) THEN
          ...
          IF(ASSOCIATED(geometricField)) THEN            
            ...
            IF(ASSOCIATED(BOUNDARY_CONDITIONS)) THEN
              ...
              DO variable_idx=1,dependentField%numberOfVariables !U and deludeln
  ...
  !                                     do nothing, tbd
                                ...
                                global_ny=FIELD_VARIABLE%domainMapping%localToGlobalMap(local_ny)

                                IF(variable_type==FIELD_U_VARIABLE_TYPE.AND.DOMAIN_NODES%NODES(node_idx)%boundaryNode) THEN !For Dirichlet
...
999 ERRORS("Poisson_EquationsSetPressurePoissonSetup",err,error)
  ...
  SUBROUTINE POISSON_PRE_SOLVE_UPDATE_PPE_MESH(CONTROL_LOOP,SOLVER,err,error,*)
    ...
    INTEGER(INTG) :: numberOfDimensions_ALE_PPE,GEOMETRIC_MESH_COMPONENT
              ...
              IF(SOLVER%SOLVE_TYPE==SOLVER_LINEAR_TYPE) THEN
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_ALE_PPE)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_ALE_PPE)) THEN
                    ...
                    END IF
                    !Get the data
                    CALL Field_NumberOfComponentsGet(EQUATIONS_SET_ALE_PPE%GEOMETRY%geometricField, & 
                      & FIELD_U_VARIABLE_TYPE,numberOfDimensions_ALE_PPE,err,error,*999)
!\todo: Introduce flags set by the user (42/1 only for testings purpose)
                ...
                CALL Field_ComponentMeshComponentGet(EQUATIONS_SET_ALE_PPE%GEOMETRY%geometricField, & 
                  & FIELD_U_VARIABLE_TYPE,1,GEOMETRIC_MESH_COMPONENT,err,error,*999)
!                 CALL Field_ParameterSetDataGet(INDEPENDENT_FIELD_ALE_PPE,FIELD_U_VARIABLE_TYPE, & 
!                   & FIELD_MESH_DISPLACEMENT_SET_TYPE,MESH_DISPLACEMENT_VALUES,err,error,*999)
                              ...
                              IF(ASSOCIATED(DOMAIN_NODES)) THEN
                                ...
                                DO node_idx=1,DOMAIN_NODES%numberOfNodes
                                  DO deriv_idx=1,DOMAIN_NODES%NODES(node_idx)%numberOfDerivatives
                                    ...
                                    CALL FieldVariable_LocalNodeDOFGet(FIELD_VARIABLE,1,deriv_idx,node_idx, &
                                      & component_idx,local_ny,err,error,*999)
                                    CALL Field_ParameterSetAddLocalDOF(EQUATIONS_SET_ALE_PPE%GEOMETRY%geometricField, & 
                ...
                END IF
                CALL Field_ParameterSetUpdateStart(EQUATIONS_SET_ALE_PPE%GEOMETRY%geometricField, & 
                  & FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,err,error,*999)
                CALL Field_ParameterSetUpdateFinish(EQUATIONS_SET_ALE_PPE%GEOMETRY%geometricField, & 
                  & FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,err,error,*999)
                ...
                CALL Field_ParameterSetsCopy(INDEPENDENT_FIELD_ALE_PPE,FIELD_U_VARIABLE_TYPE, & 
...
!     TYPE(DomainNodesType), POINTER :: DOMAIN_NODES
    ...
    INTEGER(INTG) :: I,numberOfDimensions_PPE,numberOfDimensions_FITTED,GEOMETRIC_MESH_COMPONENT
              ...
              ELSE IF(SOLVER%globalNumber==2) THEN
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_FITTED)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_FITTED)) THEN
                    ...
                    END IF
                    CALL Field_NumberOfComponentsGet(EQUATIONS_SET_FITTED%GEOMETRY%geometricField,FIELD_U_VARIABLE_TYPE, &
                        & numberOfDimensions_FITTED,err,error,*999)
                  ELSE
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_PPE)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_PPE)) THEN
                    ...
                    END IF
                    CALL Field_NumberOfComponentsGet(EQUATIONS_SET_PPE%GEOMETRY%geometricField, & 
                      & FIELD_U_VARIABLE_TYPE,numberOfDimensions_PPE,err,error,*999)
                  ELSE
...
999 ERRORSEXITS("POISSON_PRE_SOLVE_UPDATE_PPE_SOURCE",err,error)
  ...
  SUBROUTINE Poisson_EquationsSetLinearSourceSetup(EQUATIONS_SET,EQUATIONS_SET_SETUP,err,error,*)
    ...
    ENTERS("Poisson_EquationsSetLinearSourceSetup",err,error,*999)
...
999 ERRORS("Poisson_EquationsSetLinearSourceSetup",err,error)
  ...
  SUBROUTINE Poisson_EquationsSetExtracellularBidomainSetup(EQUATIONS_SET,EQUATIONS_SET_SETUP,err,error,*)
    ...
    INTEGER(INTG) :: component_idx,GEOMETRIC_COMPONENT_NUMBER,GEOMETRIC_SCALING_TYPE,numberOfDimensions, &
      ...
      & SOURCE_FIELD_NUMBER_OF_VARIABLES
    TYPE(FieldType), POINTER :: ANALYTIC_FIELD,DEPENDENT_FIELD,geometricField
    TYPE(DecompositionType), POINTER :: GEOMETRIC_DECOMPOSITION
    TYPE(EquationsType), POINTER :: equations
    TYPE(EquationsMappingVectorType), POINTER :: vectorMapping
    TYPE(EquationsMatricesVectorType), POINTER :: vectorMatrices
        ...
        CASE(EQUATIONS_SET_SETUP_DEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            ELSE
              ...
              CALL Field_DependentTypeCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_DEPENDENT_TYPE,err,error,*999)
              CALL Field_NumberOfVariablesCheck(EQUATIONS_SET_SETUP%FIELD,2,err,error,*999)
              CALL Field_VariableTypesCheck(EQUATIONS_SET_SETUP%FIELD, &              
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              ...
              ELSE
                ...
                CALL Field_NumberOfComponentsGet(EQUATIONS_SET%GEOMETRY%geometricField,FIELD_U_VARIABLE_TYPE, &
                  & numberOfDimensions,err,error,*999)
                IF(numberOfDimensions==1) THEN
                  CALL Field_NumberOfComponentsCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,2,ERR, &
                    & ERROR,*999)
                ELSEIF(numberOfDimensions==2) THEN
                  CALL Field_NumberOfComponentsCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,6,ERR, &
                    & ERROR,*999)
                ELSEIF(numberOfDimensions==3) THEN
                  CALL Field_NumberOfComponentsCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,12,ERR, &
                    & ERROR,*999)
                ENDIF              
              ENDIF
            ELSE
            ...
            CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
              ...
              ELSE
                localError="The third equations set specification of "// &
                  & TRIM(NumberToVString(EQUATIONS_SET%SPECIFICATION(3),"*",err,error))//" is invalid."
                CALL FlagError(localError,err,error,*999)
              ENDIF    
              CALL EquationsMapping_VectorCreateFinish(vectorMapping,err,error,*999)
              !Create the equations matrices
...
999 ERRORS("Poisson_EquationsSetExtracellularBidomainSetup",err,error)
  ...
  SUBROUTINE Poisson_EquationsSetNonlinearSourceSetup(EQUATIONS_SET,EQUATIONS_SET_SETUP,err,error,*)
    ...
    ENTERS("Poisson_EquationsSetNonlinearSourceSetup",err,error,*999)
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              ...
              ELSE
                ...
                CALL Field_NumberOfComponentsGet(EQUATIONS_SET%GEOMETRY%geometricField,FIELD_U_VARIABLE_TYPE, &
                  & numberOfDimensions,err,error,*999)
                IF(EQUATIONS_SET%SPECIFICATION(3)==EQUATIONS_SET_QUADRATIC_SOURCE_POISSON_SUBTYPE) THEN
                  CALL Field_NumberOfComponentsCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,numberOfDimensions+3, &
                    & err,error,*999)
                ELSE
                  CALL Field_NumberOfComponentsCheck(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,numberOfDimensions+3, &
                    & err,error,*999)
...
999 ERRORSEXITS("POISSON_EQUATION_PROBLEM_SETUP",err,error)
  ...
  SUBROUTINE POISSON_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,err,error,*)
    ...
    REAL(DP) :: CONDUCTIVITY_SUM_MATERIAL(3,3),CONDUCTIVITY_SUM(3,3),CONDUCTIVITY_SUM_TEMP(3,3),dNudX(3,3),dXdNu(3,3), &
      & DNUDXI(3,3),DXIDNU(3,3)
    ...
    TYPE(BasisType), POINTER :: DEPENDENT_BASIS,GEOMETRIC_BASIS,SOURCE_BASIS,INDEPENDENT_BASIS
    TYPE(EquationsType), POINTER :: equations
    TYPE(EquationsMappingVectorType), POINTER :: vectorMapping
    ...
    TYPE(FieldVariableType), POINTER :: FIELD_VARIABLE,SOURCE_FIELD_VARIABLE
    ...
    INTEGER(INTG) :: numberOfDimensions,NUMBER_OF_XI,NUMBER_OF_ELEMENT_PARAMETERS,node_idx,dof_idx
...
!!TODO: CHANGE ELEMENT CALCULATE TO WORK OF ns ???
              DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                ...
                IF(sourceVector%updateVector) THEN
                  ...
                  SUM=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,NO_PART_DERIV)
                  sourceVector%elementVector%vector(mhs)=sourceVector%elementVector%vector(mhs)+ & !This needs to changed to the source vector
                     & SUM*QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)*RWG
                ENDIF
              ENDDO !ms
            ENDDO !mh
          ENDDO !ng
        ...
        CASE(EQUATIONS_SET_EXTRACELLULAR_BIDOMAIN_POISSON_SUBTYPE)
          ...
          CALL Field_VariableGet(sourceField,FIELD_U_VARIABLE_TYPE,SOURCE_FIELD_VARIABLE,err,error,*999)
          
          DEPENDENT_BASIS=>dependentField%DECOMPOSITION%DOMAIN(dependentField%decomposition%meshComponentNumber)%ptr% &
            & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
          SOURCE_BASIS=>sourceField%DECOMPOSITION%DOMAIN(sourceField%decomposition%meshComponentNumber)%ptr% &
            & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
          GEOMETRIC_BASIS=>geometricField%DECOMPOSITION%DOMAIN(geometricField%decomposition%meshComponentNumber)%ptr% &
            & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS   
          
          QUADRATURE_SCHEME=>DEPENDENT_BASIS%QUADRATURE%quadratureSchemeMap(BASIS_DEFAULT_QUADRATURE_SCHEME)%ptr
          NUMBER_OF_ELEMENT_PARAMETERS=DEPENDENT_BASIS%numberOfElementParameters
                  
          numberOfDimensions=EQUATIONS_SET%REGION%coordinateSystem%numberOfDimensions
          NUMBER_OF_XI=dependentField%DECOMPOSITION%DOMAIN(dependentField%decomposition%meshComponentNumber)%ptr%TOPOLOGY% &
            & ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS%numberOfXi
          ...
          DO ng=1,QUADRATURE_SCHEME%numberOfGauss
            ...
            CALL Field_InterpolatedPointMetricsCalculate(GEOMETRIC_BASIS%numberOfXi,geometricInterpPointMetrics, &
              & err,error,*999)
              
            !Calculate RWG.
            RWG=geometricInterpPointMetrics%jacobian*QUADRATURE_SCHEME%gaussWeights(ng)
            ...
            IF(numberOfDimensions==2) THEN
              !interpolated values
              CONDUCTIVITY_I_MATERIAL(1,1)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,1)
              CONDUCTIVITY_I_MATERIAL(2,2)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,1)
              CONDUCTIVITY_I_MATERIAL(1,2)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,1)
              CONDUCTIVITY_I_MATERIAL(2,1)=CONDUCTIVITY_I_MATERIAL(1,2)
                            
              CONDUCTIVITY_SUM_MATERIAL(1,1)=CONDUCTIVITY_I_MATERIAL(1,1)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(4,1)
              CONDUCTIVITY_SUM_MATERIAL(2,2)=CONDUCTIVITY_I_MATERIAL(2,2)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(5,1)
              CONDUCTIVITY_SUM_MATERIAL(1,2)=CONDUCTIVITY_I_MATERIAL(1,2)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(6,1)
              CONDUCTIVITY_SUM_MATERIAL(2,1)=CONDUCTIVITY_SUM_MATERIAL(1,2)
            ELSE
              !interpolated values
              CONDUCTIVITY_I_MATERIAL(1,1)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,1)
              CONDUCTIVITY_I_MATERIAL(2,2)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,1)
              CONDUCTIVITY_I_MATERIAL(3,3)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,1)
              CONDUCTIVITY_I_MATERIAL(1,2)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(4,1)
              CONDUCTIVITY_I_MATERIAL(2,1)=CONDUCTIVITY_I_MATERIAL(1,2)
              CONDUCTIVITY_I_MATERIAL(2,3)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(5,1)
              CONDUCTIVITY_I_MATERIAL(3,2)=CONDUCTIVITY_I_MATERIAL(2,3)
              CONDUCTIVITY_I_MATERIAL(1,3)=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(6,1)
              CONDUCTIVITY_I_MATERIAL(3,1)=CONDUCTIVITY_I_MATERIAL(1,3)              
              
              CONDUCTIVITY_SUM_MATERIAL(1,1)=CONDUCTIVITY_I_MATERIAL(1,1)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(7,1)
              CONDUCTIVITY_SUM_MATERIAL(2,2)=CONDUCTIVITY_I_MATERIAL(2,2)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(8,1)
              CONDUCTIVITY_SUM_MATERIAL(3,3)=CONDUCTIVITY_I_MATERIAL(3,3)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(9,1)
              CONDUCTIVITY_SUM_MATERIAL(1,2)=CONDUCTIVITY_I_MATERIAL(1,2)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(10,1)
              CONDUCTIVITY_SUM_MATERIAL(2,1)=CONDUCTIVITY_SUM_MATERIAL(1,2)
              CONDUCTIVITY_SUM_MATERIAL(2,3)=CONDUCTIVITY_I_MATERIAL(2,3)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(11,1)
              CONDUCTIVITY_SUM_MATERIAL(3,2)=CONDUCTIVITY_SUM_MATERIAL(2,3)
              CONDUCTIVITY_SUM_MATERIAL(1,3)=CONDUCTIVITY_I_MATERIAL(1,3)+ &
                & equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(12,1)
            ...
            CALL CoordinateSystem_MaterialSystemCalculate(geometricInterpPointMetrics,FIBRE_INTERPOLATED_POINT, &
              & dNudX,dXdNu,DNUDXI,DXIDNU,err,error,*999)

            CALL MatrixProduct(DNUDXI,CONDUCTIVITY_SUM_MATERIAL,CONDUCTIVITY_SUM_TEMP,err,error,*999)
            CALL MatrixProduct(CONDUCTIVITY_SUM_TEMP,DXIDNU,CONDUCTIVITY_SUM,err,error,*999)

            CALL MatrixProduct(DNUDXI,CONDUCTIVITY_I_MATERIAL,CONDUCTIVITY_I_TEMP,err,error,*999)
            CALL MatrixProduct(CONDUCTIVITY_I_TEMP,DXIDNU,CONDUCTIVITY_I,err,error,*999)
                ...
                DO ms=1,NUMBER_OF_ELEMENT_PARAMETERS
                  ...
                  DO nh=1,FIELD_VARIABLE%numberOfComponents
                    DO ns=1,NUMBER_OF_ELEMENT_PARAMETERS
                      ...
                      SUM=0.0_DP
                      DO I=1,DEPENDENT_BASIS%numberOfXi
                        DO K=1,DEPENDENT_BASIS%numberOfXi
                          DO H=1,DEPENDENT_BASIS%numberOfXi
                            SUM=SUM+CONDUCTIVITY_SUM(I,K)*PGNSI(K)*PGMSI(H)*geometricInterpPointMetrics%GU(I,H)
                          ENDDO !H
                        ENDDO !K
                      ENDDO !I
                  ...
                  DO nh=1,FIELD_VARIABLE%numberOfComponents
                    DO ns=1,NUMBER_OF_ELEMENT_PARAMETERS
                      ...
                      DO I=1,DEPENDENT_BASIS%numberOfXi
                        DO K=1,DEPENDENT_BASIS%numberOfXi
                          DO H=1,DEPENDENT_BASIS%numberOfXi
                            SUM=SUM+CONDUCTIVITY_I(I,K)*PGNSI(K)*PGMSI(H)*geometricInterpPointMetrics%GU(I,H)
                          ENDDO !H
                        ENDDO !K
                      ENDDO !I
            ...
            ! initialise element rhsVectors with 0             
          ...
          IF(sourceVector%updateVector) THEN
            ...
            DO nhs=1,NUMBER_OF_ELEMENT_PARAMETERS
              ...
              node_idx=sourceVector%elementVector%rowDOFS(nhs)
              !get corresponding dof_idx (in this case node_idx=dof_idx as just 1 component, but leave for generality)
              CALL FieldVariable_LocalNodeDOFGet(SOURCE_FIELD_VARIABLE,1,1,node_idx,1,dof_idx,err,error,*999)
               !get the correct value from global source field
              CALL Field_ParameterSetGetLocalDOF(sourceField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
        ...
        CASE(EQUATIONS_SET_NONLINEAR_PRESSURE_POISSON_SUBTYPE,EQUATIONS_SET_LINEAR_PRESSURE_POISSON_SUBTYPE, &
          ...
          IF(EQUATIONS_SET%SPECIFICATION(3)==EQUATIONS_SET_ALE_PRESSURE_POISSON_SUBTYPE) THEN
            ...
            INDEPENDENT_BASIS=>independentField%DECOMPOSITION%DOMAIN(independentField% &
              & decomposition%meshComponentNumber)%ptr% &
              & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
            CALL Field_InterpolationParametersElementGet(FIELD_MESH_VELOCITY_SET_TYPE,ELEMENT_NUMBER,equations%interpolation% &
              & independentInterpParameters(FIELD_VAR_TYPE)%ptr,err,error,*999)
          ENDIF
          ...
          DO I=1,geometricField%decomposition%domain(1)%ptr%topology%elements%maximumNumberOfElementParameters
            element_node_identity=geometricField%decomposition%domain(1)%ptr%topology% &
              & elements%elements(ELEMENT_NUMBER)%elementNodes(I)
...
!             DIFF_COEFF1=0.0_DP
          ...
          DO ng=1,QUADRATURE_SCHEME%numberOfGauss
            ...
            IF(EQUATIONS_SET%SPECIFICATION(3)==EQUATIONS_SET_ALE_PRESSURE_POISSON_SUBTYPE) THEN
              ...
              IF(INDEPENDENT_BASIS%numberOfXi==3) THEN
...
! ! !               W_VALUE=0.0_DP
            ...
            U_SECOND=0.0_DP
            IF(sourceVector%updateVector) THEN
              CALL Field_InterpolationParametersElementGet(FIELD_INPUT_DATA1_SET_TYPE,ELEMENT_NUMBER, & 
              ...
              CALL Field_InterpolateGauss(SECOND_PART_DERIV,BASIS_DEFAULT_QUADRATURE_SCHEME,ng,equations%interpolation% &
                & sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr,err,error,*999)
              U_VALUE(1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,NO_PART_DERIV)
              U_VALUE(2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,NO_PART_DERIV)
              U_DERIV(1,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1)
              U_DERIV(1,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S2)
              U_DERIV(2,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1)
              U_DERIV(2,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S2)
              U_SECOND(1,1,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1_S1)
              U_SECOND(1,1,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1_S2)
              U_SECOND(1,2,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1_S2)
              U_SECOND(1,2,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S2_S2)
              U_SECOND(2,1,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1_S1)
              U_SECOND(2,1,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1_S2)
              U_SECOND(2,2,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1_S2)
              U_SECOND(2,2,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S2_S2)
              IF(DEPENDENT_BASIS%numberOfXi==3) THEN
                U_VALUE(3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,NO_PART_DERIV)
                U_DERIV(1,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S3)
                U_DERIV(2,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S3) 
                U_DERIV(3,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1)
                U_DERIV(3,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S2)
                U_DERIV(3,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S3)
                U_SECOND(1,1,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1_S3)
                U_SECOND(1,2,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S2_S3)
                U_SECOND(1,3,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S1_S3)
                U_SECOND(1,3,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S2_S3)
                U_SECOND(1,3,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,PART_DERIV_S3_S3)
                U_SECOND(2,1,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1_S3)
                U_SECOND(2,2,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S2_S3)
                U_SECOND(2,3,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S1_S3)
                U_SECOND(2,3,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S2_S3)
                U_SECOND(2,3,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,PART_DERIV_S3_S3)
                U_SECOND(3,1,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1_S1)
                U_SECOND(3,1,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1_S2)
                U_SECOND(3,1,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1_S3)
                U_SECOND(3,2,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1_S2)
                U_SECOND(3,2,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S2_S2)
                U_SECOND(3,2,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S2_S3)
                U_SECOND(3,3,1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S1_S3)
                U_SECOND(3,3,2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S2_S3)
                U_SECOND(3,3,3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,PART_DERIV_S3_S3)
              ENDIF
              CALL Field_InterpolationParametersElementGet(FIELD_INPUT_DATA2_SET_TYPE,ELEMENT_NUMBER, & 
                & equations%interpolation%sourceInterpParameters(FIELD_U_VARIABLE_TYPE)%ptr,err,error,*999)
              CALL Field_InterpolateGauss(SECOND_PART_DERIV,BASIS_DEFAULT_QUADRATURE_SCHEME,ng,equations%interpolation% &
                & sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr,err,error,*999)
              U_OLD(1)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(1,NO_PART_DERIV)
              U_OLD(2)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(2,NO_PART_DERIV)
              P_DERIV(1)=equations%interpolation%dependentInterpPoint(FIELD_VAR_TYPE)%ptr%VALUES(1,PART_DERIV_S1)
              P_DERIV(2)=equations%interpolation%dependentInterpPoint(FIELD_VAR_TYPE)%ptr%VALUES(1,PART_DERIV_S2)
              IF(DEPENDENT_BASIS%numberOfXi==3) THEN
                U_OLD(3)=equations%interpolation%sourceInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(3,NO_PART_DERIV)
                P_DERIV(3)=equations%interpolation%dependentInterpPoint(FIELD_VAR_TYPE)%ptr%VALUES(1,PART_DERIV_S3)
              ENDIF
            ENDIF
              ...
              DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                ...
                IF(equationsMatrix%updateMatrix) THEN
                  ...
                  DO nh=1,FIELD_VARIABLE%numberOfComponents
                    DO ns=1,DEPENDENT_BASIS%numberOfElementParameters
                      nhs=nhs+1
                      PHINS=QUADRATURE_SCHEME%gaussBasisFunctions(ns,NO_PART_DERIV,ng)
                      DO ni=1,DEPENDENT_BASIS%numberOfXi
                        PGMSI(ni)=QUADRATURE_SCHEME%gaussBasisFunctions(ms,PARTIAL_DERIVATIVE_FIRST_DERIVATIVE_MAP(ni),ng)
                        PGNSI(ni)=QUADRATURE_SCHEME%gaussBasisFunctions(ns,PARTIAL_DERIVATIVE_FIRST_DERIVATIVE_MAP(ni),ng)
                        DO mi=1,DEPENDENT_BASIS%numberOfXi
                          DXI_DX(mi,ni)=equations%interpolation%geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE)%ptr%dXidX(mi,ni)
                        END DO
                      ENDDO !ni
                      SUM=0.0_DP
                      ...
                      DO mi=1,DEPENDENT_BASIS%numberOfXi
                        DO ni=1,DEPENDENT_BASIS%numberOfXi
                          SUM=SUM+PGMSI(mi)*PGNSI(ni)*equations%interpolation% &
                            & geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE)%ptr%GU(mi,ni)*DIFF_COEFF1
                        ENDDO !ni
                      ENDDO !mi
                      equationsMatrix%elementMatrix%matrix(mhs,nhs)=equationsMatrix%elementMatrix%matrix(mhs,nhs)+SUM*RWG
                    ENDDO !ns
...
!                       this is the second viscous term
                ...
                ELSE IF(EQUATIONS_SET%SPECIFICATION(3)==EQUATIONS_SET_NONLINEAR_PRESSURE_POISSON_SUBTYPE.OR. &
                  ...
                  IF(sourceVector%updateVector) THEN
                    ...
                    ENDIF
                    sourceVector%elementVector%vector(mhs)=sourceVector%elementVector%vector(mhs)+SUM*RWG &
                      & -SUM2*QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)*RWG
                  ENDIF
                ELSE
                  sourceVector%elementVector%vector(mhs)=0.0_DP
                ENDIF
              ENDDO !ms
            ENDDO !mh
          ENDDO !ng
            ...
            DO mh=1,FIELD_VARIABLE%numberOfComponents
              ...
              DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                ...
                IF(rhsVector%updateVector) rhsVector%elementVector%vector(mhs)=rhsVector%elementVector%vector(mhs)* &
                  & equations%interpolation%dependentInterpParameters(FIELD_VAR_TYPE)%ptr%scaleFactors(ms,mh)
                IF(sourceVector%updateVector) sourceVector%elementVector%vector(mhs)=sourceVector% &
                  & elementVector%vector(mhs)*equations%interpolation%dependentInterpParameters(FIELD_VAR_TYPE)% &
                  & PTR%scaleFactors(ms,mh)
              ENDDO !ms
            ENDDO !mh
          ENDIF
        CASE(EQUATIONS_SET_LINEAR_SOURCE_POISSON_SUBTYPE)
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        CASE(EQUATIONS_SET_QUADRATIC_SOURCE_POISSON_SUBTYPE)
          ...
          IF(jacobianMatrix%updateJacobian) THEN
            ...
            FIELD_VAR_TYPE=DEPENDENT_VARIABLE%variableType
              ...
              DO mh=1,DEPENDENT_VARIABLE%numberOfComponents
                ...
                DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                  ...
                  DO nh=1,DEPENDENT_VARIABLE%numberOfComponents
                    DO ns=1,DEPENDENT_BASIS%numberOfElementParameters
                      nhs=nhs+1
                      VALUE=-2.0_DP*C_PARAM*QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)* &
                        & QUADRATURE_SCHEME%gaussBasisFunctions(ns,NO_PART_DERIV,ng)*U_VALUE
              ...
              DO mh=1,DEPENDENT_VARIABLE%numberOfComponents
                ...
                DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                  ...
                  DO nh=1,DEPENDENT_VARIABLE%numberOfComponents
                    DO ns=1,DEPENDENT_BASIS%numberOfElementParameters
                      nhs=nhs+1
                      VALUE=-B_PARAM*C_PARAM*QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)* &
                        & QUADRATURE_SCHEME%gaussBasisFunctions(ns,NO_PART_DERIV,ng)*EXP(C_PARAM*U_VALUE)
                      jacobianMatrix%elementJacobian%matrix(mhs,nhs)=jacobianMatrix%elementJacobian%matrix(mhs,nhs)+VALUE*RWG
                    ENDDO !ns
                  ENDDO !nh
                ENDDO !ms
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        CASE(EQUATIONS_SET_QUADRATIC_SOURCE_POISSON_SUBTYPE)                 
          ...
          DO ng=1,QUADRATURE_SCHEME%numberOfGauss
            ...
            !Calculate RWG.
!!TODO: Think about symmetric problems.
            WG=QUADRATURE_SCHEME%gaussWeights(ng)
            RWG=equations%interpolation%geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE)%ptr%jacobian*WG
            !Loop over field components
                        ...
                        DO nj=1,GEOMETRIC_VARIABLE%numberOfComponents
                          ...
                          DO ni=1,GEOMETRIC_BASIS%numberOfXi
                            ...
                            PGNJ(nj)=PGNJ(nj)+ &
                              ...
                              & equations%interpolation%geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE)%ptr%dXidX(ni,nj)
                          ENDDO !ni
                          K_PARAM=equations%interpolation%materialsInterpPoint(FIELD_U_VARIABLE_TYPE)%ptr%VALUES(nj,NO_PART_DERIV)
                          SUM1=SUM1+K_PARAM*PGMJ(nj)*PGNJ(nj)
                        ENDDO !nj
                        SUM2=B_PARAM*QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)* &
                          & QUADRATURE_SCHEME%gaussBasisFunctions(ns,NO_PART_DERIV,ng)                        
                        equationsMatrix%elementMatrix%matrix(mhs,nhs)=equationsMatrix%elementMatrix%matrix(mhs,nhs)+ &
                          & (SUM1+SUM2)*RWG
                      ENDDO !ns
                    ENDDO !nh
                ...
                DO mh=1,DEPENDENT_VARIABLE%numberOfComponents
                  ...
                  DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                    ...
                    rhsVector%elementVector%vector(mhs)=rhsVector%elementVector%vector(mhs)+ &
                       & QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)*A_PARAM*RWG
                  ENDDO !ms
                ENDDO !mh
              ENDIF
            ENDIF
              ...
              DO mh=1,DEPENDENT_VARIABLE%numberOfComponents
                ...
                DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                  ...
                  nonlinearMatrices%elementResidual%vector(mhs)=nonlinearMatrices%elementResidual%vector(mhs)- &
                    & QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)*C_PARAM*U_VALUE**2*RWG
                ENDDO !ms
              ENDDO !mh
            ENDIF
          ENDDO !ng
                    ...
                    DO nh=1,DEPENDENT_VARIABLE%numberOfComponents
                      DO ns=1,DEPENDENT_BASIS%numberOfElementParameters
                        ...
                        DO nj=1,GEOMETRIC_VARIABLE%numberOfComponents
                          ...
                          SUM1=SUM1+K_PARAM*PGMJ(nj)*PGNJ(nj)
                        ENDDO !nj
                        equationsMatrix%elementMatrix%matrix(mhs,nhs)=equationsMatrix%elementMatrix%matrix(mhs,nhs)+SUM1*RWG
                      ENDDO !ns
                    ENDDO !nh
                  ENDDO !ms
                ENDDO !mh
...
!!TODO: Handle floating point exceptions better
              ...
              DO mh=1,DEPENDENT_VARIABLE%numberOfComponents
                ...
                DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                  mhs=mhs+1
                  nonlinearMatrices%elementResidual%vector(mhs)=nonlinearMatrices%elementResidual%vector(mhs)- &
                    & QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)*B_PARAM*EXP(C_PARAM*U_VALUE)*RWG
                ENDDO !ms
              ENDDO !mh
            ENDIF
          ENDDO !ng
...
999 ERRORSEXITS("Poisson_ProblemNonlinearSourceSetup",err,error)
  ...
  SUBROUTINE Poisson_PostSolve(solver,err,error,*)
    ...
    ENTERS("oisson_PostSolve",err,error,*999)
    ...
    IF(SIZE(problem%specification,1)<3) &
      & CALL FlagError("Problem specification must have at least three entries for a Poission problem.",err,error,*999)
    ...
    CASE(PROBLEM_LINEAR_PRESSURE_POISSON_SUBTYPE,PROBLEM_NONLINEAR_PRESSURE_POISSON_SUBTYPE, &
      & PROBLEM_ALE_PRESSURE_POISSON_SUBTYPE)
      CALL POISSON_POST_SOLVE_OUTPUT_DATA(controlLoop,solver,err,error,*999)
    CASE(PROBLEM_FITTED_PRESSURE_POISSON_SUBTYPE)
  ...
  SUBROUTINE Poisson_PreSolve(solver,err,error,*)
            ...
            IF(ASSOCIATED(EQUATIONS_SET)) THEN
              IF(ASSOCIATED(EQUATIONS_SET%ANALYTIC)) THEN
                ...
                ELSE
                  ...
                  CALL POISSON_PRE_SOLVE_UPDATE_INPUT_DATA(CONTROLLOOP,SOLVER,err,error,*999)
    ...
    CASE(PROBLEM_FITTED_PRESSURE_POISSON_SUBTYPE)
      ...
      ELSE IF(SOLVER%globalNumber==2) THEN
        ...
        CALL POISSON_PRE_SOLVE_UPDATE_PPE_SOURCE(CONTROLLOOP,SOLVER,err,error,*999)
    ...
    CASE(PROBLEM_ALE_PRESSURE_POISSON_SUBTYPE)
      IF(CONTROLLOOP%whileLoop%iterationNumber==1)THEN
        ...
        ENDIF
        !Update mesh
        CALL POISSON_PRE_SOLVE_UPDATE_PPE_MESH(CONTROLLOOP,SOLVER,err,error,*999)
      ENDIF
    CASE DEFAULT
      localError="Problem subtype "//TRIM(NumberToVString(problem%specification(3),"*",err,error))// &
        & " is not valid for a Poisson type of a classical field problem class."
  ...
  !          FILE="TRANSIENT_OUTPUT"
                          ...
                          IF(EXPORT_FIELD) THEN          
                            IF(MOD(CURRENT_LOOP_ITERATION,OUTPUT_ITERATION_NUMBER)==0)  THEN   
                              ...
                              CALL FLUID_MECHANICS_IO_WRITE_ENCAS_BLOCK(EQUATIONS_SET%REGION,EQUATIONS_SET%globalNumber, &
