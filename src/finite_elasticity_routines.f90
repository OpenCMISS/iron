...
999 ERRORSEXITS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR",ERR,ERROR)
  ...
  !>Evaluates the spatial elasticity and stress tensor in Voigt form at a given Gauss point.
  SUBROUTINE FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR_ISOCHORIC_VOLUMETRIC(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
      & MATERIALS_INTERPOLATED_POINT,ELASTICITY_TENSOR,STRESS_TENSOR,DZDNU,Jznu,ELEMENT_NUMBER,GAUSS_POINT_NUMBER,ERR,ERROR,*)
    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER, INTENT(IN) :: EQUATIONS_SET !<A pointer to the equations set
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: DEPENDENT_INTERPOLATED_POINT,MATERIALS_INTERPOLATED_POINT
    ...
    ENTERS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR_ISOCHORIC_VOLUMETRIC",ERR,ERROR,*999)
    ...
    EXITS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR_ISOCHORIC_VOLUMETRIC")
    RETURN
999 ERRORSEXITS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR_ISOCHORIC_VOLUMETRIC",ERR,ERROR)
    RETURN 1

  END SUBROUTINE FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR_ISOCHORIC_VOLUMETRIC
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        IF(JACOBIAN_MATRIX%UPDATE_JACOBIAN) THEN
          IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
            DEPENDENT_FIELD=>EQUATIONS%INTERPOLATION%GEOMETRIC_FIELD
            GEOMETRIC_FIELD=>EQUATIONS%INTERPOLATION%DEPENDENT_FIELD
          ELSE
          ...
          IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
            GEOMETRIC_INTERP_POINT=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT(FIELD_VAR_TYPE)%PTR
            GEOMETRIC_INTERP_POINT_METRICS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS(FIELD_VAR_TYPE)%PTR
            DEPENDENT_INTERP_POINT=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT(FIELD_U_VARIABLE_TYPE)%PTR
            DEPENDENT_INTERP_POINT_METRICS=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE)%PTR
          ELSE
...
999 ERRORSEXITS("FINITE_ELASTICITY_PUSH_STRESS_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FiniteElasticity_StrainTensor(deformationGradientTensor,rightCauchyDeformationTensor,fingerDeformationTensor, &
    ...
    REAL(DP) :: deformationGradientTensorT(3,3) !<The elastic part of the  deformation gradient tensor
  ...
  SUBROUTINE FiniteElasticity_FiniteElementResidualEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: DEPENDENT_COMPONENT_INTERPOLATION_TYPE, COMPONENT_INTERPOLATION_TYPE
    ...
    REAL(DP) :: DZDNU(3,3),DZDNUT(3,3),AZL(3,3),AZU(3,3),Fe(3,3),FeT(3,3),Fg(3,3),C(3,3),f(3,3),E(3,3), &
      & I3,P,piolaTensor(3,3),TEMP(3,3),dZdXi(3,3)
	...
	IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
        ...
        FIELD_VAR_TYPE=FIELD_VARIABLE%VARIABLE_TYPE
	IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
          GEOMETRIC_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_PARAMETERS(FIELD_VAR_TYPE)%PTR
          DEPENDENT_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_PARAMETERS(FIELD_U_VARIABLE_TYPE)%PTR
        ELSE
...
!       END IF
        ...
        !Point interpolation pointer
        IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
          GEOMETRIC_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT(FIELD_U_VARIABLE_TYPE)%PTR
          GEOMETRIC_INTERPOLATED_POINT_METRICS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE)%PTR
          DEPENDENT_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT(FIELD_VAR_TYPE)%PTR
          DEPENDENT_INTERPOLATED_POINT_METRICS=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT_METRICS(FIELD_VAR_TYPE)%PTR
        ELSE
...
!           ENDIF
        ...
        CASE(EQUATIONS_SET_CONSTITUTIVE_LAW_IN_CELLML_EVALUATE_SUBTYPE, &
          ...
          DO gauss_idx=1,DEPENDENT_NUMBER_OF_GAUSS_POINTS
            ...
            DO component_idx=1,NUMBER_OF_DIMENSIONS
              IF(EQUATIONS_SET_SUBTYPE==EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
                DEPENDENT_COMPONENT_INTERPOLATION_TYPE=GEOMETRIC_FIELD%VARIABLES(var1)%COMPONENTS(component_idx)%INTERPOLATION_TYPE
              ELSE
        ...
        !Scale factor adjustment
        IF(DEPENDENT_FIELD%SCALINGS%SCALING_TYPE/=FIELD_NO_SCALING) THEN
          CALL Field_InterpolationParametersScaleFactorsElementGet(ELEMENT_NUMBER,DEPENDENT_INTERPOLATION_PARAMETERS,err,error,*999)
          mhs=0
		...
		IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
                  IF (mh > 3) THEN
                    !Scale factors for hydrostatic pressure components. 
                    NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)=NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)* &
                      & GEOMETRIC_INTERPOLATION_PARAMETERS%SCALE_FACTORS(ms,mh)
                  ELSE
                    NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)=NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)* &
                      & DEPENDENT_INTERPOLATION_PARAMETERS%SCALE_FACTORS(ms,mh)
                  ENDIF
...
999 ERRORS("FiniteElasticityEquationsSet_DerivedVariableCalculate",err,error)
  ...
  SUBROUTINE FiniteElasticity_StrainCalculate(equationsSet,strainField,strainFieldVariableType,err,error,*)
    ...
    INTEGER(INTG) :: var1 ! Variable number corresponding to 'U' in single physics case
    INTEGER(INTG) :: var2 ! Variable number corresponding to 'DELUDLEN' in single physics case
    REAL(DP) :: DZDNU(3,3),E(3,3),AZL(3,3),DZDNUT(3,3)
    REAL(DP) :: Jxxi
    ...
    IF(tensorEvaluateType==EQUATIONS_SET_EVALUATE_CAUCHY_STRESS_TENSOR) THEN
      ...
      CASE(EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE)
        ...
        CALL FINITE_ELASTICITY_GAUSS_STRESS_TENSOR(equationsSet,dependentInterpolatedPoint, &
          & materialsInterpolatedPoint,geometricInterpolatedPoint,cauchyStressVoigt,dZdNu,Jznu,localElementNumber,0,ERR,ERROR,*999)
      ...
      CASE(EQUATIONS_SET_ORTHOTROPIC_MATERIAL_COSTA_SUBTYPE, EQUATIONS_SET_TRANSVERSE_ISOTROPIC_GUCCIONE_SUBTYPE, &
        ...
        CALL FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR(equationsSet,dependentInterpolatedPoint, &
          & materialsInterpolatedPoint,geometricInterpolatedPoint,darcyInterpolatedPoint, &
...
999 ERRORS("FiniteElasticity_SurfacePressureJacobianEvaluate",ERR,ERROR)
  ...
  SUBROUTINE FiniteElasticity_SurfacePressureResidualEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,var1,var2,ERR,ERROR,*)
    ...
    TYPE(EQUATIONS_MATRICES_NONLINEAR_TYPE), POINTER :: NONLINEAR_MATRICES
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD
    TYPE(FIELD_INTERPOLATION_PARAMETERS_TYPE), POINTER :: FACE_DEPENDENT_INTERPOLATION_PARAMETERS, DEPENDENT_INTERPOLATION_PARAMETERS
    ...
    DO element_face_idx=1,NUMBER_OF_LOCAL_FACES
      ...
      IF(DECOMP_FACE%BOUNDARY_FACE) THEN !!temporary until MESH_FACE (or equivalent) is available (decomp face includes ghost faces?)
        ...
        IF(NONZERO_PRESSURE) THEN
          ...
          IF (EQUATIONS_SET_SUBTYPE == EQUATIONS_SET_REFERENCE_STATE_TRANSVERSE_GUCCIONE_SUBTYPE) THEN
            FACE_DEPENDENT_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_PARAMETERS(FIELD_VAR_U_TYPE)%PTR
            FACE_DEPENDENT_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT(FIELD_VAR_U_TYPE)%PTR
            FACE_DEPENDENT_INTERPOLATED_POINT_METRICS=>EQUATIONS%INTERPOLATION% &
              & GEOMETRIC_INTERP_POINT_METRICS(FIELD_VAR_U_TYPE)%PTR
          ELSE
        ...
        CASE(PROBLEM_SETUP_SOLVERS_TYPE)
          ...
          CASE(PROBLEM_SETUP_START_ACTION)
            ...
            ELSE
              ...
              IF(PROBLEM%SPECIFICATION(3)==PROBLEM_FINITE_ELASTICITY_CELLML_SUBTYPE) THEN
                !Create the CellML evaluator solver
                CALL SOLVER_NEWTON_CELLML_EVALUATOR_CREATE(SOLVER,CELLML_SOLVER,ERR,ERROR,*999)
                !Link the CellML evaluator solver to the solver
                CALL SOLVER_LINKED_SOLVER_ADD(SOLVER,CELLML_SOLVER,SOLVER_CELLML_EVALUATOR_TYPE,ERR,ERROR,*999)
