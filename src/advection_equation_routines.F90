...
CONTAINS
  ...
  SUBROUTINE Advection_EquationsSetSetup(EQUATIONS_SET,equationsSetSetup,err,error,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_ADVECTION_SUBTYPE)
        CALL Advection_EquationsSetLinearSetup(EQUATIONS_SET,equationsSetSetup,err,error,*999)
      CASE DEFAULT
...
999 ERRORSEXITS("Advection_EquationsSetSpecificationSet",err,error)
  ...
  SUBROUTINE Advection_EquationsSetLinearSetup(EQUATIONS_SET,equationsSetSetup,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error
    !Local Variables
    INTEGER(INTG) :: GEOMETRIC_MESH_COMPONENT,GEOMETRIC_SCALING_TYPE,GEOMETRIC_COMPONENT_NUMBER,numberOfDimensions
        ...
        CASE(EQUATIONS_SET_SETUP_DEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            IF(EQUATIONS_SET%DEPENDENT%dependentFieldAutoCreated) THEN
              ...
              !label the field
              CALL Field_LabelSet(EQUATIONS_SET%dependent%dependentField,"Concentration",err,error,*999)
            ...
            ELSE 
              ...
              CALL Field_NumberOfVariablesCheck(equationsSetSetup%field,DEPENDENT_FIELD_NUMBER_OF_VARIABLES,err,error,*999)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              IF(EQUATIONS_MATERIALS%materialsFieldAutoCreated) THEN
                ...
                CALL Field_CreateStart(equationsSetSetup%fieldUserNumber,EQUATIONS_SET%REGION, & 
                  & EQUATIONS_SET%MATERIALS%materialsField,err,error,*999)
                ...
                CALL Field_VariableTypesSetAndLock(EQUATIONS_MATERIALS%materialsField,[FIELD_U_VARIABLE_TYPE],err,error,*999)
...
999 ERRORSEXITS("Advection_ProblemSpecificationSet",err,error)
  ...
  SUBROUTINE ADVECTION_EQUATION_PROBLEM_SETUP(PROBLEM,problemSetup,err,error,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_ADVECTION_SUBTYPE)
        CALL ADVECTION_EQUATION_PROBLEM_LINEAR_SETUP(PROBLEM,problemSetup,err,error,*999)
      CASE DEFAULT
...
999 ERRORSEXITS("ADVECTION_EQUATION_PROBLEM_SETUP",err,error)
  ...
  SUBROUTINE ADVECTION_EQUATION_PROBLEM_LINEAR_SETUP(PROBLEM,problemSetup,err,error,*)
...
999 ERRORSEXITS("ADVECTION_EQUATION_PROBLEM_LINEAR_SETUP",err,error)
  ...
  SUBROUTINE ADVECTION_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,err,error,*)
    ...
    TYPE(EquationsVectorType), POINTER :: vectorEquations
    TYPE(FieldType), POINTER :: DEPENDENT_FIELD,INDEPENDENT_FIELD,GEOMETRIC_FIELD,MATERIALS_FIELD
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        CASE(EQUATIONS_SET_ADVECTION_SUBTYPE)
          ...
          DEPENDENT_BASIS=>DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(GEOMETRIC_FIELD%decomposition%meshComponentNumber)%ptr% &
            & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
          ...
          CASE(EQUATIONS_SET_ADVECTION_SUBTYPE)
            ...
            DO ng=1,QUADRATURE_SCHEME%numberOfGauss
              ...
              DO xiIdx=1,DEPENDENT_BASIS%numberOfXi
                DO coordIdx=1,equations%interpolation%geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE) &
                  ...
                  DXI_DX=DXI_DX+(equations%interpolation%geometricInterpPointMetrics(FIELD_U_VARIABLE_TYPE)% &
                    & PTR%dXidX(xiIdx,coordIdx))**2.0_DP
                END DO !coordIdx
              END DO !xiIdx
              DXI_DX=SQRT(DXI_DX)
              ...
              DO ms=1,DEPENDENT_BASIS%numberOfElementParameters
                PHIMS=QUADRATURE_SCHEME%gaussBasisFunctions(ms,NO_PART_DERIV,ng)
                DPHIMS_DXI=QUADRATURE_SCHEME%gaussBasisFunctions(ms,FIRST_PART_DERIV,ng)
                mhs=mhs+1
                nhs=0
                IF(updateStiffnessMatrix .OR. updateDampingMatrix) THEN
                  ...
                  DO ns=1,DEPENDENT_BASIS%numberOfElementParameters
                    ...
                    DPHINS_DXI=QUADRATURE_SCHEME%gaussBasisFunctions(ns,FIRST_PART_DERIV,ng)
                    nhs=nhs+1
              ...
              IF(ASSOCIATED(SOLVER_MAPPING)) THEN
                ...
                IF(ASSOCIATED(EQUATIONS)) THEN
                  ...
                  IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
                    ...
                    CALL Field_ParameterSetUpdateLocalDOF(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
