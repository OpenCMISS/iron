    ...
    CASE(EQUATIONS_SET_CHARACTERISTIC_SUBTYPE)
       ...
       CASE(EQUATIONS_SET_SETUP_MATERIALS_TYPE)
          ...
          CASE(EQUATIONS_SET_CHARACTERISTIC_SUBTYPE)
             ...
             CASE(EQUATIONS_SET_SETUP_START_ACTION)
                ...
                IF(.NOT.ASSOCIATED(equationsMaterials)) CALL FlagError("Equations set materials is not associated.",err,error,*999)
       ...
       CASE(EQUATIONS_SET_SETUP_EQUATIONS_TYPE)
          ...
          CASE(EQUATIONS_SET_CHARACTERISTIC_SUBTYPE)
             ...
             CASE(EQUATIONS_SET_SETUP_START_ACTION)
                equationsMaterials=>equationsSet%MATERIALS
                IF(.NOT.ASSOCIATED(equationsMaterials)) CALL FlagError("Equations materials is not associated.",err,error,*999)
...
999 ERRORSEXITS("Characteristic_NodalJacobianEvaluate",err,error)
  ...
  SUBROUTINE Characteristic_Extrapolate(solver,timeIncrement,err,error,*)
    ...
    IF(.NOT.ASSOCIATED(solverEquations)) CALL FlagError("Solvers is not associated.",err,error,*999)
    ...
    IF(.NOT.ASSOCIATED(equationsSet)) CALL FlagError("Solver equations are not associated.",err,error,*999)
    equations=>equationsSet%EQUATIONS
    ...
    DO nodeIdx=1,numberOfLocalNodes
       ...
       IF(numberOfVersions>1 .OR. boundaryConditionType>0) THEN
          ...
          DO i=1,numberOfVersions
             ...
             IF(lambda(i)*normalWave(i)<ZERO_TOLERANCE) CALL FlagError("Subcritical 1D system violated.",err,error,*999)
             ...
             IF(lambda(i)*normalWave(i)<-ZERO_TOLERANCE)  CALL FLAG_ERROR("Subcritical 1D system violated.",err,error,*999)

             IF(.NOT. overExtrapolated) &
