...
#include "macros.h"  
  ...
  TYPE BASIS_INFO_TYPE
    TYPE(BasisType), POINTER :: BASIS !<The OpenCMISS basis.
    INTEGER(INTG) :: CONNECTIVITY_HANDLE !<The basis connectivity evaluator handle.
...
999 ERRORSEXITS( "FIELDML_OUTPUT_GET_XI_TYPE", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_GET_VALUE_TYPE( FML_HANDLE, FIELD, VARIABLE_TYPE, DO_IMPORT, TYPE_HANDLE, ERR, ERROR, * )
    ...
    !Locals
    INTEGER(INTG) :: FIELDTYPE_, SUB_TYPE, COUNT
    ...
    CALL Field_NumberOfComponentsGet( FIELD, VARIABLE_TYPE, COUNT, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FieldmlOutputGetSimplexConnectivityType", err, error )
  ...
  SUBROUTINE FIELDML_OUTPUT_GET_CONNECTIVITY_ENSEMBLE( FIELDML_HANDLE, BASIS, TYPE_HANDLE, ERR, ERROR, * )
    !Argument variables
    INTEGER(INTG), INTENT(IN) :: FIELDML_HANDLE !<The FieldML session handle.
    TYPE(BasisType), POINTER :: BASIS !<The basis for which to return the connectivity.
    INTEGER(INTG), INTENT(OUT) :: TYPE_HANDLE !<The FieldML connectivity ensemble.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string

    !Locals
    INTEGER(INTG) :: FIELDBASISTYPE, XI_COUNT
    ...
    CALL Basis_TypeGet( BASIS, FIELDBASISTYPE, ERR, ERROR, *999 )
    CALL Basis_NumberOfXiGet( BASIS, XI_COUNT, ERR, ERROR, *999 )
    
    IF( FIELDBASISTYPE == BASIS_LAGRANGE_HERMITE_TP_TYPE ) THEN
      ...
      CALL Basis_InterpolationXiGet( BASIS, XI_INTERPOLATIONS, ERR, ERROR, *999 )
      CALL Basis_CollapsedXiGet( BASIS, COLLAPSE_INFO, ERR, ERROR, *999 )
    ...
    ELSE IF( FIELDBASISTYPE == BASIS_SIMPLEX_TYPE ) THEN
      ALLOCATE( XI_INTERPOLATIONS( XI_COUNT ), STAT = ERR )
      IF( ERR /= 0 ) CALL FlagError( "Could not allocate xi interpolations array.", ERR, ERROR, *999 )
      CALL Basis_InterpolationXiGet( BASIS, XI_INTERPOLATIONS, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_OUTPUT_FIND_LAYOUT", ERR, ERROR )
  ...
  FUNCTION FIELDML_OUTPUT_FIND_BASIS( BASIS_INFO, BASIS, ERR, ERROR )
    ...
    TYPE(BasisType), POINTER, INTENT(IN) :: BASIS !<The basis handle to search for. 
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
...
999 ERRORSEXITS( "FIELDML_OUTPUT_GET_SIMPLE_BASIS_NAME", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_CREATE_BASIS_REFERENCE( FIELDML_INFO, BASE_NAME, BASIS_INFO, ERR, ERROR, * )
    ...
    CALL Basis_TypeGet( BASIS_INFO%BASIS, BASIS_TYPE, ERR, ERROR, *999 )
    CALL Basis_NumberOfXiGet( BASIS_INFO%BASIS, XI_COUNT, ERR, ERROR, *999 )
    ...
    IF( BASIS_TYPE == BASIS_LAGRANGE_HERMITE_TP_TYPE ) THEN
      ...
      ALLOCATE( COLLAPSE_INFO( XI_COUNT ), STAT = ERR )
      CALL Basis_InterpolationXiGet( BASIS_INFO%BASIS, XI_INTERPOLATIONS, ERR, ERROR, *999 )
      CALL Basis_CollapsedXiGet( BASIS_INFO%BASIS, COLLAPSE_INFO, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_OUTPUT_CREATE_LAYOUT_PARAMETERS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_MESH_COMPONENT( FIELDML_INFO, BASE_NAME, CONNECTIVITY_FORMAT, COMPONENT_NUMBER, &
    ...
    DO I = 1, ELEMENT_COUNT
      NULLIFY(BASIS)
      CALL MeshElements_ElementBasisGet( MESH_ELEMENTS, I, BASIS, ERR, ERROR, *999 )
    ...
    DO I = 1, CONNECTIVITY_COUNT
      ...
      DO J = 1, ELEMENT_COUNT
        NULLIFY(BASIS)
        CALL MeshElements_ElementBasisGet( MESH_ELEMENTS, J, BASIS, ERR, ERROR, *999 )
  
        CALL FIELDML_OUTPUT_GET_CONNECTIVITY_ENSEMBLE( FIELDML_INFO%FML_HANDLE, BASIS, LAYOUT_HANDLE, ERR, ERROR, *999 )
        IF( LAYOUT_HANDLE == CONNECTIVITY_INFO(I)%LAYOUT_HANDLE ) THEN
          CALL MeshElements_ElementNodesGet( MESH_ELEMENTS, J, IBUFFER, ERR, ERROR, *999 )
        ELSE
          IBUFFER(:) = 0
        ENDIF
...
999 ERRORSEXITS( "FIELDML_OUTPUT_ADD_MESH_COMPONENT", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_NODE_DOFS( FIELDML_INFO, BASE_NAME, DOF_FORMAT, TYPE_HANDLE, FIELD, &
    ...
    DO I = 1, COMPONENT_COUNT
      CALL Field_ComponentMeshComponentGet( FIELD, VARIABLE_TYPE, FIELD_COMPONENT_NUMBERS(I), &
        & MESH_COMPONENT_NUMBERS(I), ERR, ERROR, *999 )
      CALL Field_ComponentInterpolationGet( FIELD, VARIABLE_TYPE, FIELD_COMPONENT_NUMBERS(I), INTERPOLATION_TYPE, &
        & ERR, ERROR, *999 )
      ...
      DO J = 1, COMPONENT_COUNT
        ...
        IF( IS_NODE_BASED(J) ) THEN
          ...
          IF( NODE_EXISTS ) THEN
            ...
            CALL Decomposition_MeshComponentNumberGet(FIELD%DECOMPOSITION,meshComponentNumber,err,error,*999)
            CALL Decomposition_NodeDomainGet(FIELD%DECOMPOSITION,I,meshComponentNumber,nodeDomain,err,error,*999)
            IF(nodeDomain==myGroupComputationNodeNumber) THEN
              CALL Field_ParameterSetGetNode( FIELD, VARIABLE_TYPE, SET_TYPE, VERSION_NUMBER, &
    ...
    DO I = 1, ELEMENT_COUNT
      DO J = 1, COMPONENT_COUNT
        ...
        IF( IS_ELEMENT_BASED(J) ) THEN
          CALL Field_ParameterSetGetElement( FIELD, VARIABLE_TYPE, SET_TYPE, I, &
...
999 ERRORSEXITS( "FIELDML_OUTPUT_ADD_FIELD_ELEMENT_DOFS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_CONSTANT_DOFS( FIELDML_INFO, BASE_NAME, DOF_FORMAT, TYPE_HANDLE, FIELD, &
    ...
    CALL Field_DataTypeGet( FIELD, VARIABLE_TYPE, DATA_TYPE, ERR, ERROR, *999 )
    ...
    IF( IS_REAL ) THEN
      ...
      DO J = 1, COMPONENT_COUNT
        ...
        IF( IS_CONSTANT(J) ) THEN
          CALL Field_ParameterSetGetConstant( FIELD, VARIABLE_TYPE, SET_TYPE, &
            & FIELD_COMPONENT_NUMBERS(J), DVALUE, ERR, ERROR, *999 )
        ENDIF
        DBUFFER( J ) = DVALUE
      ENDDO
...
999 ERRORSEXITS( "FIELDML_OUTPUT_ADD_FIELD_CONSTANT_DOFS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_INITIALISE_INFO( MESH, LOCATION, BASE_NAME, CONNECTIVITY_FORMAT, FIELDML_INFO, ERR, ERROR, * )
    !Argument variables
    TYPE(MeshType), POINTER, INTENT(IN) :: MESH !<The mesh with which the FieldML document is associated.
    ...
    DIMENSIONS = MESH%numberOfDimensions
...
!      & fieldmlInfo, err, errorString, *999 )
    ...
    CALL Mesh_NumberOfComponentsGet( MESH, COMPONENT_COUNT, ERR, ERROR, *999 )
    DO I = 1, COMPONENT_COUNT
      NULLIFY(meshTopology)
      CALL Mesh_MeshTopologyGet(MESH,I,meshTopology,err,error,*999)
      NULLIFY( MESH_ELEMENTS )
      CALL LIST_ITEM_ADD( FIELDML_INFO%COMPONENT_HANDLES, FML_INVALID_HANDLE, ERR, ERROR, *999 )
      CALL MeshTopology_MeshElementsGet(meshTopology,MESH_ELEMENTS, ERR, ERROR, *999 )
...
999 ERRORSEXITS( "FIELDML_OUTPUT_ADD_FIELD_NO_TYPE", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_WITH_TYPE( FIELDML_INFO, BASE_NAME, DOF_FORMAT, FIELD, VARIABLE_TYPE, SET_TYPE, &
    ...
    CALL Field_NumberOfComponentsGet( FIELD, VARIABLE_TYPE, COMPONENT_COUNT, ERR, ERROR, *999 )
