...
999 ERRORSEXITS("EQUATIONS_SET_BACKSUBSTITUTE",err,error)
  ...
  !>Backsubstitutes with an equations set to calculate unknown right hand side vectors for the nonlinear case
  !>This is a draft!!! Still gives
  !>Computes the BS for ALL rhs dofs: select case free/fixed/mixed should be uncommented (sparse case)
  SUBROUTINE EQUATIONS_SET_NONLINEAR_BACKSUBSTITUTE(equationsSet,BOUNDARY_CONDITIONS,err,error,*)
    ...
    TYPE(EquationsType), POINTER :: equations
    TYPE(EquationsMappingVectorType), POINTER :: vectorMapping
    TYPE(EquationsMappingNonlinearType), POINTER :: linearMapping
    TYPE(EquationsMappingRHSType), POINTER :: rhsMapping
    TYPE(EquationsMappingSourceType), POINTER :: sourceMapping
    TYPE(EquationsMatricesVectorType), POINTER :: vectorMatrices
    TYPE(EquationsMatricesDynamicType), POINTER :: dynamicMatrices
    TYPE(EquationsMatricesNonlinearType), POINTER :: linearMatrices
    TYPE(EquationsMatricesSourceType), POINTER :: sourceVector
    TYPE(EquationsJacobianType), POINTER :: equationsMatrix
    TYPE(EquationsVectorType), POINTER :: vectorEquations
    TYPE(FIELD_TYPE), POINTER :: dependentField
    TYPE(FIELD_VARIABLE_TYPE), POINTER :: DEPENDENT_VARIABLE,rhsVariable
    TYPE(VARYING_STRING) :: localError
    ...
    ENTERS("EQUATIONS_SET_NONLINEAR_BACKSUBSTITUTE",err,error,*999)
          ...
          IF(ASSOCIATED(EQUATIONS)) THEN
            ...
            IF(ASSOCIATED(vectorMatrices)) THEN
              ...
              IF(ASSOCIATED(dynamicMatrices)) THEN
                !CALL FlagError("Not implemented.",err,error,*999)
              ELSE
                linearMatrices=>vectorMatrices%nonlinearMatrices
                IF(ASSOCIATED(linearMatrices)) THEN
                  vectorMapping=>vectorEquations%vectorMapping
                  IF(ASSOCIATED(vectorMapping)) THEN
                    linearMapping=>vectorMapping%nonlinearMapping
                    IF(ASSOCIATED(linearMapping)) THEN
                      rhsMapping=>vectorMapping%rhsMapping
                      sourceMapping=>vectorMapping%sourceMapping
                      IF(ASSOCIATED(rhsMapping)) THEN
                        IF(ASSOCIATED(BOUNDARY_CONDITIONS)) THEN
                          ...
                          IF(ASSOCIATED(rhsVariable)) THEN
                            ...
                            IF(ASSOCIATED(RHS_DOMAIN_MAPPING)) THEN
                              ...
                              IF(ASSOCIATED(RHS_BOUNDARY_CONDITIONS)) THEN
                                !Loop over the equations matrices
                                DO equations_matrix_idx=1,linearMatrices%numberOfJacobians
                                  DEPENDENT_VARIABLE=>linearMapping%jacobianToVarMap(equations_matrix_idx)%VARIABLE
                                  IF(ASSOCIATED(DEPENDENT_VARIABLE)) THEN
                                    VARIABLE_TYPE=DEPENDENT_VARIABLE%VARIABLE_TYPE
                                    !Get the dependent field variable parameters
                                    CALL Field_ParameterSetDataGet(dependentField,VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                                      & DEPENDENT_PARAMETERS,err,error,*999)
                                    equationsMatrix=>linearMatrices%jacobians(equations_matrix_idx)%ptr
                                    IF(ASSOCIATED(equationsMatrix)) THEN
                                      COLUMN_DOMAIN_MAPPING=>linearMapping%jacobianToVarMap(equations_matrix_idx)% &
                                        & columnDOFSMapping
                                      IF(ASSOCIATED(COLUMN_DOMAIN_MAPPING)) THEN
                                        EQUATIONS_DISTRIBUTED_MATRIX=>equationsMatrix%jacobian
                                        IF(ASSOCIATED(EQUATIONS_DISTRIBUTED_MATRIX)) THEN
                                          CALL DistributedMatrix_StorageTypeGet(EQUATIONS_DISTRIBUTED_MATRIX, &
                                            & EQUATIONS_STORAGE_TYPE,err,error,*999)
                                          CALL DistributedMatrix_DataGet(EQUATIONS_DISTRIBUTED_MATRIX,equationsMatrixData, &
    ...
    EXITS("EQUATIONS_SET_NONLINEAR_BACKSUBSTITUTE")
    RETURN
999 ERRORSEXITS("EQUATIONS_SET_NONLINEAR_BACKSUBSTITUTE",err,error)
    RETURN 1
   
  END SUBROUTINE EQUATIONS_SET_NONLINEAR_BACKSUBSTITUTE
