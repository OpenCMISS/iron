...
998 ERRORSEXITS("EQUATIONS_INITIALISE",err,error)
  ...
  SUBROUTINE EQUATIONS_INTERPOLATION_FINALISE(EQUATIONS_INTERPOLATION,err,error,*)
    ...
    IF(ASSOCIATED(EQUATIONS_INTERPOLATION)) THEN
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%GEOMETRIC_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%FIBRE_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%DEPENDENT_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%prevDependentInterpParameters,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%INDEPENDENT_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%MATERIALS_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolationParametersFinalise(EQUATIONS_INTERPOLATION%SOURCE_INTERP_PARAMETERS,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%GEOMETRIC_INTERP_POINT,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%DEPENDENT_INTERP_POINT,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%prevDependentInterpPoint,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%INDEPENDENT_INTERP_POINT,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%FIBRE_INTERP_POINT,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%MATERIALS_INTERP_POINT,err,error,*999)
      CALL Field_InterpolatedPointsFinalise(EQUATIONS_INTERPOLATION%SOURCE_INTERP_POINT,err,error,*999)
      CALL Field_PhysicalPointsFinalise(EQUATIONS_INTERPOLATION%DEPENDENT_PHYSICAL_POINT,err,error,*999)
      CALL Field_InterpolatedPointsMetricsFinalise(EQUATIONS_INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS,err,error,*999)
      CALL Field_InterpolatedPointsMetricsFinalise(EQUATIONS_INTERPOLATION%prevDependentInterpPointMetrics,err,error,*999)
