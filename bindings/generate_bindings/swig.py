...
def routine_swig_lines(routine):
    ...
    if routine.name.endswith('_Initialise') and routine.name != 'cmfe_Initialise':
        type = routine.name[0:-len('_Initialise')] + 'Type'
        name = routine.parameters[0].name
        start_lines.append(
            '%%apply cmfe_DummyInitialiseType *cmfe_Dummy{%s *%s};' %
            (type, name))
        end_lines.append('%%clear %s *%s;' % (type, name))
    elif routine.name.endswith('_Finalise') and routine.name != 'cmfe_Finalise':
        type = routine.name[0:-len('_Finalise')] + 'Type'
        name = routine.parameters[0].name
        start_lines.append(
            '%%apply cmfe_DummyFinaliseType *cmfe_Dummy{%s *%s};'
            % (type, name))
        end_lines.append('%%clear %s *%s;' % (type, name))
        ...
        if parameter.var_type == Parameter.CHARACTER:
            ...
            if parameter.var_type == Parameter.CUSTOM_TYPE:
                ...
                typemap = ('const int ArraySize, '
                    'const cmfe_DummyType *DummyTypes')
            ...
            typemap = 'const cmfe_Bool DummyInputBool'
            apply_to = 'const cmfe_Bool *%(name)s' % properties
    if typemap:
